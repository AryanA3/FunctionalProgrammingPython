...
def create_html_table(data_rows):
    rows = accumulate_rows(list(map(lambda x: accumulate_data_cells(x), data_rows)))
        
    def create_table_headers(headers):
        nonlocal rows
        h = tag_row(''.join(map(lambda x : tag_header(x), headers)))
        rows = h + rows
...
def create_html_table_given_solution(data_rows):
